...
### A standing rule: reports are not diffs

Whenever an existing report gets regenerated — a new-cycle `README.md`, a refreshed `deep-dives/` file, a re-run of one of the ad-hoc `$CANDIDATE_DIR/reports/*.md` write-ups — **write it as a fresh, standalone document for a first-time reader, not as a changelog against whatever came before.** Left to its own devices, an LLM re-running a report loves to narrate its own history ("the previous version found $X, this version found $Y because Z was fixed") right in the report body. Don't. A reader opening the file has no reason to care about an earlier draft they never saw; give them the current, best-known picture of the numbers and move on. If a prior pass genuinely matters — a bug that changed a total, a data refresh that extended the date range — say so in one line in the Methodology & AI Transparency section, in past tense, and leave it out of the report's actual findings entirely. This applies no matter which prompt template below produced the report, and no matter whether the rerun was prompted by fresh data, a tooling fix, or a user pointing out something the last pass missed.

### A standing rule: connected committees are opt-in

A committee's `fec/<id>/` directory can carry a `CONNECTED` marker file — dropped there by hand when the committee is financially tied to the candidate but is **not** one of their own (an externally-run, multi-candidate joint fundraising committee that happened to earmark money to the candidate, for example). See gotcha 9 in [`/tooling/analyze-candidate.rb`](tooling/analyze-candidate.rb)'s header for the full reasoning and how the tool handles it; the short version is that `analyze-candidate.rb` already keeps CONNECTED-marked committees out of every Key Donors/Major Spending/Takeaways number, but still surfaces them in their own "CONNECTED COMMITTEES" section of its output.

**Treat that section as opt-in for a written report, the same way the tool treats it as excluded from the totals by default.** Don't narrate, summarize, or otherwise fold a CONNECTED committee's numbers into a `README.md` or deep-dive write-up by default, even though the data is sitting right there in the tool's output — leave it out entirely unless the user's prompt for that specific run explicitly asks for it. The concern isn't accuracy (the tool's exclusion already keeps it out of the real totals); it's that this data is second-order by nature — a JFC's own fundraising mostly serves other candidates, and explaining the earmarking mechanics correctly takes real space — and defaulting to including it would dilute a report that's supposed to be about the candidate's own committees. This is a different bucket than "Suggested Committees for Further Investigation" below: a CONNECTED committee is already collected and itemized on disk, just deliberately held out of the narrative, where a suggested committee hasn't been collected at all yet. A standalone report format specifically for connected-committee data may get built someday; until then, treat it as available-but-silent.
...
**Prompt v8** (also folds in raw efile data to cover any gap past the processed export's own coverage — see "Prompt history (v7)" below; v8 adds an explicit no-diff-narration instruction — see "Prompt history (v8)" below)
...
**Prompt history (v8):** Several `$CANDIDATE_DIR/reports/` write-ups regenerated in the same session as a data refresh or a tooling fix (a keyword-scan expansion, an efile-gap bug fix) kept spending a chunk of the new report explaining how the new numbers differed from the old ones — "the earlier version found $X, this version found $Y" — right in the body a reader would see. That's a changelog, not a report, and it's exactly the instinct "A standing rule: reports are not diffs" (above, under "Step 2: Analyze") now exists to head off. v8 doesn't change anything about how `analyze-candidate.rb` is run; it adds one instruction to this prompt's own Methodology section (below) making that standing rule concrete for this specific template, so a fresh session re-running this prompt for a new cycle — or a user regenerating an existing report after a fix — doesn't have to be told twice.
