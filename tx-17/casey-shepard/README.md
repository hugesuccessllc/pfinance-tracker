...
5. **One donor's location doesn't match the district.** Kristen Young's $200 contribution lists a Leander, TX address — outside TX-17 entirely. It's a minor, plausible detail (an out-of-district friend or former colleague giving a small amount) rather than a red flag, but it's the only donor in the file who isn't local to the Waco-area district Shepard is running in.
...
- **Directory naming note:** the candidate's own FEC filings and this repository's directory both spell the name "Casey Shepard" (one "p"); the request that produced this report used "Casey Sheppard" (two). Per this repo's directory-matching convention, the existing close-but-not-exact `tx-17/casey-shepard/` directory was used rather than creating a new one.
- **Command run:**
  ```bash
  ruby tooling/analyze-candidate.rb \
    ...
    --cycle 2026
  ```
  `--house-ethics-dir` was omitted: no `house-ethics/` directory exists yet for this candidate — it has not been collected, not merely found empty. House candidates are generally required to file a Financial Disclosure Statement with the Clerk of the House once they cross $5,000 in contributions or expenditures (a threshold this committee has already passed), so a filing likely exists; it just hasn't been gathered here yet. That's a Step 1 gap for a human (or a separately-tasked LLM session) to close, not something this analysis pass should fill in.
- **Data provenance:** `fec/C00934547/` carries `.download-progress` and `.efile-progress` marker files and a `PRINCIPAL` marker, indicating it was collected via `fec-api-client.rb --download` rather than manually exported, with the itemized schedule_a/schedule_b files timestamped 2026-09-03.
...
- **Source:** FEC disclosures in `tx-17/casey-shepard/fec/`, filtered to the 2026 cycle. No House Ethics Committee data was available to include (see above).
