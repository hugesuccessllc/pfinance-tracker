...
- District boundaries were verified against Wikipedia's [Texas's 31st](https://en.wikipedia.org/wiki/Texas's_31st_congressional_district) and [Texas's 11th](https://en.wikipedia.org/wiki/Texas's_11th_congressional_district) congressional district articles rather than assumed from the seat's pre-redistricting shape, at the user's direct request. The 2026-effective map consolidates Fort Cavazos and all of Bell/Coryell counties into TX-31 (TX-11's article states its new map removes Bell County "entirely, eliminating Fort Cavazos" and the parts of the district that stretched to Killeen), while TX-11 instead picks up parts of suburban Austin, Cedar Park, Leander, and Pflugerville. The specific detail that only "small chunks" of Round Rock/Cedar Park/Leander (not the bulk of those cities) move to TX-11 comes from the user directly, not from the Wikipedia article itself, which doesn't break the new map down to city level — see the equivalent note recorded against this repo's TX-11 candidate analyses.
